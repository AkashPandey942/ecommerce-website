            ...
            <motion.div key="payment" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="w-full max-w-[515px] flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-6 pt-4">
                ...
                <div className="flex gap-4">
                  <div className="flex-1 space-y-1"><label className="text-sm opacity-60">Expiry</label><input type="text" placeholder="MM/YY" className="bg-white/5 border border-white/20 h-11 rounded-md px-4 w-full outline-none" /></div>
                  <div className="flex-1 space-y-1"><label className="text-sm opacity-60">CVV</label><input type="text" placeholder="123" className="bg-white/5 border border-white/20 h-11 rounded-md px-4 w-full outline-none" /></div>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full h-11 bg-[#7C4DFF] hover:bg-[#6B3EEB] rounded-lg font-bold shadow-lg mt-4 cursor-pointer flex items-center justify-center gap-2 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
          ...
          {currentStep === 2 && (
            <motion.div key="confirmation" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-[488px] flex flex-col items-center gap-8">
              ...
              <div className="flex gap-4">
                {paymentStatus === 'failed' && (
                  <button
                    onClick={() => { setPaymentStatus(null); setErrorMessage(null); setCurrentStep(1); }}
              ...
              <div className="w-full bg-white/[0.1] border border-white/20 rounded-[30px] p-8">
                ...
                <div className="flex justify-between font-bold text-lg">
                  ...
                  <span className={paymentStatus === 'failed' ? 'text-[#CF4141]' : 'text-[#98BD42]'}>Rs. 14,160</span>
                </div>
              </div>
            </motion.div>
          )}
