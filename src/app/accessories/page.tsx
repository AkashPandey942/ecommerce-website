        ...
        <section className="mt-8 mb-10">
          ...
          >
            ...
            <p className="font-roboto font-normal text-base text-[#C2C6D6]">
              Choose the type of accessory you want to generate
            </p>
          </motion.div>
        </section>
        ...
        <section className="mb-20">
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
            {styles.map((style: any, idx: number) => (
              <motion.div 
                key={idx} 
                className={`${style.fullWidth ? "col-span-2 lg:col-span-1" : "col-span-1"}`}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              >
                <Link href={`/accessories/${style.title.toLowerCase().replace(/\s+/g, '-')}`}>
                  <SegmentCard 
                    title={style.title} 
                    image={style.image} 
