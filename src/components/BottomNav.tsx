...
const BottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 h-[99px] bg-black rounded-t-[30px] shadow-[0px_-6px_38px_rgba(35,161,255,0.25)] flex lg:hidden items-center justify-around px-5 pb-[15px]">
      {items.map((item, idx) => {
        ...
        return (
          ...
          >
            <div className={`w-6 h-6 flex items-center justify-center ${isActive ? "glow-blue" : "text-[#9CA3AF]"}`}>
              <item.icon className={`w-6 h-6 ${isActive ? "text-white" : "text-[#9CA3AF]"}`} />
            </div>
